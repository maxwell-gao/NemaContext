...
def test_embryo_jepa_model_forward_and_ema_update():
    """Embryo JEPA should predict masked future target embeddings and update EMA target weights."""
    h5ad_path = Path("dataset/processed/nema_extended_large2025.h5ad")
    if not h5ad_path.exists():
        pytest.skip("Processed gene-context dataset not available")
    ...
    model = EmbryoJEPAModel(backbone=backbone, d_model=64, ema_decay=0.9)
    out = model(
        genes=genes,
        time=time,
        token_times=token_times,
    ...
    assert out.target_masked_future_latent.shape == (2, 64)
    assert out.pred_masked_future_latent.shape == (2, 64)

    target_before = next(model.target_backbone.parameters()).detach().clone()
    online_param = next(model.online_backbone.parameters())
    with torch.no_grad():
        online_param.add_(0.1)
    model.update_target_encoder()
    target_after = next(model.target_backbone.parameters()).detach()
