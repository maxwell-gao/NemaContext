...
def test_standard_spatial_loader():
    ...
    for i, cell in enumerate(test_cells[:5]):
        if not np.isnan(coords[i, 0]):
            print(
                f"  {cell}: [{coords[i, 0]:.2f}, {coords[i, 1]:.2f}, {coords[i, 2]:.2f}]"
            )
...
def test_time_dynamic_contact_graph():
    ...
    if len(all_cells) >= 2:
        ...
        nonzero = (areas > 0).sum()
        print(
            f"✓ Contact timeseries {cell1}-{cell2}: {nonzero}/{len(times)} frames with contact"
        )
    ...
    if graphs:
        ...
        print(
            f"✓ Created TimeDynamicContactGraph with {len(time_graph.time_frames)} frames"
        )
...
def test_contact_gnn():
    ...
    print(
        f"  Degree stats: mean={degree.mean():.2f}, max={degree.max()}, min={degree.min()}"
    )
    ...
    print("✓ Contact vs k-NN comparison:")
