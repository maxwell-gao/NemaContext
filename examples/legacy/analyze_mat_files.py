...
def analyze_standard_dataset_1():
    ...
    with h5py.File(mat_file, "r") as f:
        ...
        for col in range(emb0.shape[1]):
            ...
            if col_data.dtype == "object":
                ...
                if inner.dtype.kind == "u" and inner.shape[0] < 100:
                    ...
                    print(
                        f"    Col {col} ({column_names[col]}): {n_cells} cells, e.g. '{sample_str}'"
                    )
                else:
                    print(
                        f"    Col {col}: {n_cells} entries, inner shape {inner.shape}"
                    )
            else:
                data = col_data[:]
                print(
                    f"    Col {col} ({column_names[col]}): shape {col_data.shape}, values {data.flatten()[:3]}..."
                )
        ...
        for i in range(embryo_morph.shape[0]):
            ...
            emb_data = f[emb_ref]
            if (
                len(emb_data.shape) >= 2
                and emb_data.shape[0] > 0
                and emb_data.shape[1] > 0
            ):
                has_morph += 1
                ...
                for col in range(min(4, emb_data.shape[1])):
                    ...
                    if cell_data.dtype == "object" and cell_data.shape[0] > 0:
                        ...
                        if inner.dtype.kind == "u" and np.prod(inner.shape) < 100:
                            ...
                            print(
                                f"    Col {col}: inner shape {inner.shape}, dtype {inner.dtype}"
                            )
                    else:
                        print(
                            f"    Col {col}: shape {cell_data.shape}, dtype {cell_data.dtype}"
                        )
    ...
    with h5py.File(sample_file, "r") as f:
        ...
        for key in f.keys():
            ...
            if isinstance(obj, h5py.Dataset) and len(obj.shape) == 3:
                ...
                print(
                    f"  Physical dimensions: {physical_dim[0]:.1f} x {physical_dim[1]:.1f} x {physical_dim[2]:.1f} μm"
                )
...
def extract_cell_positions(frame_idx=1, embryo_idx=0):
    ...
    with h5py.File(mat_file, "r") as f:
        ...
        if embryo_idx >= embryo_positions.shape[0]:
            print(
                f"Embryo index {embryo_idx} out of range (max: {embryo_positions.shape[0] - 1})"
            )
            return None
        ...
        for i in range(min(10, n_cells)):
            print(
                f"{cell_names[i]:<20} {coords[0][i]:>10.2f} {coords[1][i]:>10.2f} {coords[2][i]:>10.2f}"
            )

        return {"names": cell_names, "x": coords[0], "y": coords[1], "z": coords[2]}
