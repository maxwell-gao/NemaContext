...
def test_morphology_loader():
    ...
    try:
        ...
        print(
            f"  Volume range: {morph['volume'].min():.1f} - {morph['volume'].max():.1f} μm³"
        )
        print(
            f"  Surface range: {morph['surface'].min():.1f} - {morph['surface'].max():.1f} μm²"
        )
        print(
            f"  Sphericity range: {morph['sphericity'].min():.3f} - {morph['sphericity'].max():.3f}"
        )
