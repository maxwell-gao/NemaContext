...
def analyze_contact_interface(data_path: Path) -> dict:
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            print(
                f"  {sample_id}: {n_timepoints} timepoints, {len(cell_names)} cells, {total_contacts} total contact entries"
            )
    ...
    for vol_file in volume_files:
        ...
        try:
            ...
            if len(vol_values) > 0:
                ...
                print(
                    f"    Volume range: {np.min(vol_values):.0f} - {np.max(vol_values):.0f} (mean: {np.mean(vol_values):.0f})"
                )
    ...
    for surf_file in surface_files:
        ...
        try:
            ...
            if len(surf_values) > 0:
                ...
                print(
                    f"  {sample_id}: Surface range {np.min(surf_values):.0f} - {np.max(surf_values):.0f}"
                )
...
def analyze_standard_dataset_1(data_path: Path) -> dict:
    ...
    results = {"n_frames": 0, "files_found": [], "cell_name_info": None}
    ...
    print(
        f"  - Dataset frames: {results['n_frames']} (WorkSpace_Dataset_1.mat to WorkSpace_Dataset_54.mat)"
    )

    # Check for cell name file
    cellname_file = sd1_dir / "WorkSpace_CellName.mat"
    if cellname_file.exists():
        print("  - Cell name mapping: WorkSpace_CellName.mat")

    if HAS_SCIPY and cellname_file.exists():
        try:
            mat_data = loadmat(str(cellname_file))
            print(
                f"    Keys in CellName file: {[k for k in mat_data.keys() if not k.startswith('__')]}"
            )
        except Exception as e:
...
def analyze_standard_dataset_2(data_path: Path) -> dict:
    ...
    print(
        f"  Frames: {len(results['frames'])} (range: {min(results['frames'])} - {max(results['frames'])})"
    )
    print(
        f"  Embryos: {len(results['embryos'])} ({', '.join(results['embryos'][:5])}...)"
    )
    print(
        f"  Expected matrix shape: {results['matrix_shape']} at {results['voxel_resolution_um']} μm/voxel"
    )

    # Physical dimensions
    dims_um = tuple(d * results["voxel_resolution_um"] for d in results["matrix_shape"])
    print(
        f"  Physical dimensions: {dims_um[0]:.1f} x {dims_um[1]:.1f} x {dims_um[2]:.1f} μm"
    )
...
def print_summary(contact_results, volume_results, sd1_results, sd2_results):
    ...
    if volume_results["cell_lineages"]:
        ...
        print("   - Cells per founder lineage:")
    ...
    if sd2_results["frames"]:
        print(
            f"   - Frames: {min(sd2_results['frames'])} - {max(sd2_results['frames'])}"
        )
