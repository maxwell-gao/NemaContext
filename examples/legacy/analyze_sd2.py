...
def main():
    ...
    print(
        f"  Expected: {len(frames)} × {len(embryos)} = {len(frames) * len(embryos)} files"
    )
    ...
    for frame in sample_frames:
        ...
        print(
            f"  Cell voxels: {result['cell_voxels']:,} / {result['total_voxels']:,} "
        ...
        if result["cells"]:
            ...
            for cell in result["cells"][:3]:
                ...
                print(
                    f"    Label {cell['label']}: {cell['voxels']:,} voxels, "
    ...
    print(
        f"Physical size: {physical_dim[0]:.1f} × {physical_dim[1]:.1f} × {physical_dim[2]:.1f} μm"
    )
    ...
    for frame in range(1, 55, 5):
        ...
        if fpath.exists():
            ...
            print(
                f"{frame:<8} {result['cell_count']:<8} {result['cell_voxels']:<15,} {occupancy:<12.1f}"
            )
