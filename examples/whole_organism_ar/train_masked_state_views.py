#!/usr/bin/env python3
"""Train masked self-supervised developmental state representations."""
...
class MaskedStateViewModel(nn.Module):
    """Shared encoder with masked-view and temporal prediction heads."""
    ...
    def __init__(
        ...
        self.masked_view_predictor = nn.Sequential(
        ...
        self.disc_projector = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),
            nn.Linear(d_model, d_model),
        )
        self.masked_future_predictor = nn.Sequential(
        ...
        gene_dim = self.encoder.encoder.gene_dim
        self.current_gene_decoder = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),
            nn.Linear(d_model, context_size * gene_dim),
        )
        self.future_gene_decoder = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),
            nn.Linear(d_model, context_size * gene_dim),
    ...
    def encode_view(self, batch: dict[str, torch.Tensor], prefix: str, genes_override: torch.Tensor | None = None):
        genes = batch[f"{prefix}_genes"] if genes_override is None else genes_override
...
def mask_view_genes(
    ...
    """Mask a random subset of gene features on valid tokens."""
    if mask_ratio <= 0.0:
        return genes, torch.zeros_like(genes, dtype=torch.bool)
    random = torch.rand(genes.shape, device=genes.device, generator=generator)
    valid = valid_mask.unsqueeze(-1).expand_as(genes)
    mask = (random < mask_ratio) & valid
    masked = genes.clone()
    masked[mask] = 0.0
    return masked, mask
...
def compute_masked_metrics(
    ...
    current_visible_genes, _current_mask = mask_view_genes(
        ...
        batch["current_view_0_valid_mask"],
        mask_ratio,
    )
    future_visible_genes, _future_mask = mask_view_genes(
        batch["future_view_0_genes"],
        batch["future_view_0_valid_mask"],
        mask_ratio,
    )

    z_current_visible, _ = model.encode_view(
        batch, "current_view_0", genes_override=current_visible_genes
    )
    z_current_target, _ = model.encode_view(batch, "current_view_1")
    z_future_target, _ = model.encode_view(batch, "future_view_0")

    masked_view_pred = model.masked_view_predictor(z_current_visible)
    masked_view_loss = (1.0 - F.cosine_similarity(masked_view_pred, z_current_target.detach(), dim=-1)).mean()
    pred_current_genes = model.current_gene_decoder(masked_view_pred).view_as(batch["current_view_1_genes"])
    current_mask_f = _current_mask.float()
    current_mask_count = current_mask_f.sum().clamp_min(1.0)
    current_gene_loss = (
        ((pred_current_genes - batch["current_view_1_genes"]) ** 2) * current_mask_f
    ).sum() / current_mask_count
    ...
    if "future_view_1_genes" in batch:
        z_future_visible, _ = model.encode_view(
            batch, "future_view_0", genes_override=future_visible_genes
        )
        z_future_masked_target, _ = model.encode_view(batch, "future_view_1")
        masked_future_input = torch.cat([z_current_visible, z_future_visible], dim=-1)
        masked_future_pred = model.masked_future_predictor(masked_future_input)
        masked_future_loss = (
            1.0 - F.cosine_similarity(masked_future_pred, z_future_masked_target.detach(), dim=-1)
        ).mean()
        ot_target_genes = batch["future_view_1_genes"]
        future_gene_target = batch["future_view_1_genes"]
    else:
        masked_future_pred = model.masked_future_predictor(torch.cat([z_current_visible, z_future_target], dim=-1))
        masked_future_loss = (
            1.0 - F.cosine_similarity(masked_future_pred, z_future_target.detach(), dim=-1)
        ).mean()
        ot_target_genes = batch["future_view_0_genes"]
        future_gene_target = batch["future_view_0_genes"]

    pred_future_masked_genes = model.future_gene_decoder(masked_future_pred).view_as(future_gene_target)
    future_mask_f = _future_mask.float()
    future_mask_count = future_mask_f.sum().clamp_min(1.0)
    future_gene_loss = (
        ((pred_future_masked_genes - future_gene_target) ** 2) * future_mask_f
    ).sum() / future_mask_count

    pred_future_genes = model.future_decoder(masked_future_pred).view(
...
def parse_args():
    p = argparse.ArgumentParser(description="Train masked self-supervised developmental state representations.")
    ...
    p.add_argument("--future_views_per_state", type=int, default=2)
...
def main():
    ...
    if not train_ds.time_pairs or not val_ds.time_pairs:
        raise ValueError("Masked state-view dataset is empty after filtering.")
