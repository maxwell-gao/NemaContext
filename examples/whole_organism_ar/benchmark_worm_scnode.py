#!/usr/bin/env python3
"""scNODE-style baseline on the worm Large2025 forecasting benchmark."""
...
class ScNODEStyleBaseline(nn.Module):
    ...
    def transition(self, z: torch.Tensor, current_time: torch.Tensor, future_time: torch.Tensor) -> torch.Tensor:
        ...
        inp = torch.cat([z, time_feat.expand(z.shape[0], z.shape[1], 2)], dim=-1)
        dz = self.time_mlp(inp)
        return z + dz

    def forward(self, current_genes: torch.Tensor, current_time: torch.Tensor, future_time: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        z = self.encoder(current_genes)
        z_future = self.transition(z, current_time, future_time)
...
def parse_args():
    ...
    p.add_argument('--split_mode', choices=['time_transition'], default='time_transition')
    ...
    p.add_argument('--output_json', default='result/gene_context/benchmark_worm_scnode_time_transition.json')
    p.add_argument('--output_csv', default='result/gene_context/benchmark_worm_scnode_time_transition_common.csv')
    return p.parse_args()
...
def split_transition_keys(dataset, eval_fraction: float):
    keys = []
    for pair in dataset.snapshot_pairs:
        keys.append(f"{pair.history_bins[-1]}->{pair.future_bin}")
    keys = sorted(set(keys), key=lambda x: tuple(int(p) for p in x.split('->')))
...
def iter_loader(dataset, indices, batch_size, device):
    subset = [dataset[i] for i in indices]
    loader = DataLoader(subset, batch_size=batch_size, shuffle=True, collate_fn=collate_large2025_whole_embryo)
    for batch in loader:
...
def compute_loss(model, batch, args):
    ...
    total = loss_sinkhorn + args.recon_weight * recon_loss + args.mean_weight * mean_loss
    return total


def train_model(dataset, train_idx, args):
    model = ScNODEStyleBaseline(dataset.gene_dim, latent_dim=args.latent_dim, hidden_dim=args.hidden_dim).to(args.device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    for _ in range(args.epochs):
        model.train()
        for batch in iter_loader(dataset, train_idx, args.batch_size, args.device):
...
def eval_records(model, dataset, indices, args):
    ...
    records = []
    with torch.no_grad():
        for batch in iter_loader(dataset, indices, args.batch_size, args.device):
            current = batch['history_genes'][:, -1]
            future = batch['future_genes']
...
def main():
    ...
    results = {
        ...
        'n_eval': len(eval_idx),
        'metrics': {
            'scnode_style': compute_metrics(model_records, top_de_idx),
