...
def parse_args():
    ...
    p.add_argument("--test_mode", default="gene_to_spatial", choices=["gene_to_spatial", "spatial_to_gene"])
...
def evaluate_gene_to_spatial(model: torch.nn.Module, dataset, n_test: int, device: str):
    ...
    with torch.no_grad():
        for i in range(min(n_test, len(dataset))):
            ...
            true_spatial = cont_states[:, dataset._gene_dim:dataset._gene_dim + dataset._spatial_dim]

            # Create dummy spatial input (zeros) to test gene->spatial prediction
            dummy_spatial = torch.zeros_like(cont_states[:, dataset._gene_dim:dataset._gene_dim + dataset._spatial_dim].to(device))
            ...
            if i < 10:  # Only for first few samples
                ...
                for gene_idx in range(0, dataset._gene_dim, 100):
                    ...
                    change = torch.norm(pred_spatial_pert - baseline_pred, dim=-1).mean().item()
...
def evaluate_spatial_to_gene(model: torch.nn.Module, dataset, n_test: int, device: str):
    ...
    with torch.no_grad():
        for i in range(min(n_test, len(dataset))):
            ...
            spatial = cont_states[:, dataset._gene_dim:dataset._gene_dim + dataset._spatial_dim].to(device)
            true_genes = cont_states[:, :dataset._gene_dim]

            # Create dummy gene input
            dummy_genes = torch.zeros_like(cont_states[:, :dataset._gene_dim].to(device))
