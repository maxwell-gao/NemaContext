...
from autoregressive_simulation import AutoregressiveSimulator, create_initial_state, BranchingState


def main():
    parser = argparse.ArgumentParser(description="Paired perturbation experiment")
    parser.add_argument("--checkpoint", type=str, default="checkpoints_trimodal_crossmodal/best.pt")
    ...
    print(f"Will delete {args.deletion_fraction*100:.0f}% at t={args.perturbation_time}")
    print()

    # Load model
    checkpoint = torch.load(args.checkpoint, map_location=args.device)
    model = CrossModalNemaModel(
        gene_dim=2000, spatial_dim=3, discrete_K=7,
        d_model=256, n_layers=6, n_heads=8, cross_modal_every=2,
    ).to(args.device)
    ...
    print(f"Initial spatial center: {initial_state.states[0][0, :, -3:].mean(dim=0).cpu().numpy()}")
    print()
    ...
    def deletion_perturb(state: BranchingState) -> BranchingState:
        ...
        print(f"  [t={args.perturbation_time}] Deleted {n_delete} cells, {n_kept} remain")
    ...
    print(f"{'Time':<8} {'Control (cells)':<20} {'Perturbed (cells)':<20} {'Distance':<15}")
    print("-" * 70)

    for i in range(0, len(ctrl_traj), max(1, len(ctrl_traj) // 10)):
        ctrl, pert = ctrl_traj[i], pert_traj[i]
        dist = np.linalg.norm(np.array(ctrl["spatial_mean"]) - np.array(pert["spatial_mean"]))
        print(f"{ctrl['t']:<8.2f} {ctrl['n_cells']:<20} {pert['n_cells']:<20} {dist:<15.4f}")

    # Final analysis
    ctrl_final = ctrl_traj[-1]
    pert_final = pert_traj[-1]
    final_dist = np.linalg.norm(np.array(ctrl_final["spatial_mean"]) - np.array(pert_final["spatial_mean"]))

    print("\n" + "=" * 70)
    print("FINAL ANALYSIS")
    print("=" * 70)
    print(f"Control:   {ctrl_final['n_cells']} cells at ({ctrl_final['spatial_mean'][0]:.2f}, {ctrl_final['spatial_mean'][1]:.2f}, {ctrl_final['spatial_mean'][2]:.2f})")
    print(f"Perturbed: {pert_final['n_cells']} cells at ({pert_final['spatial_mean'][0]:.2f}, {pert_final['spatial_mean'][1]:.2f}, {pert_final['spatial_mean'][2]:.2f})")
