"""Base multi-cell gene-context dataset built from transcriptome time windows."""
...
class GeneContextDataset(Dataset):
    ...
    def __init__(
        ...
        if delete_target_mode not in {"weak", "strict"}:
            raise ValueError(f"Unsupported delete_target_mode: {delete_target_mode}")
    ...
    def _prepare_features(self, adata: ad.AnnData, n_hvg: int):
        ...
        self.gene_std = np.clip(expr.std(axis=0), a_min=1e-6, a_max=None).astype(np.float32)
    ...
    def _select_hvg_indices(adata: ad.AnnData, n_hvg: int) -> np.ndarray:
        ...
        return np.arange(min(n_hvg, adata.n_vars))
    ...
    def _compute_targets(
        ...
        match_type = np.full(current_take, self.MATCH_UNSUPERVISED, dtype=np.int64)
        ...
        for i, lineage in enumerate(current_lineages):
            ...
            if exact_matches:
                ...
                best_local = exact_matches[int(np.argmin(np.abs(candidate_times - pair.future_center)))]
        ...
        del_target = strict_del_target if self.delete_target_mode == "strict" else weak_del_target
    ...
    def summarize_time_pair(self, pair_idx: int) -> dict[str, float | int]:
        ...
        summary["split_positive_rate"] = summary["split_positive_count"] / max(1, summary["current_count"])
        summary["del_positive_rate"] = summary["del_positive_count"] / max(1, summary["current_count"])
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.sampling_strategy == "spatial_anchor":
            current_indices, context_role, relative_position = self._select_anchor_context(pair, rng)
        else:
