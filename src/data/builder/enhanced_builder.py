...
class EnhancedAnnDataBuilder(TrimodalAnnDataBuilder):
    ...
    def _add_morphology_features(
        ...
        if ancestor_mapper is not None:
            ...
            fuzzy_matches = stats.get("fuzzy_x_matches", 0) + stats.get(
                "fuzzy_slash_matches", 0
            )
            ancestor_matches = stats.get("ancestor_matches", 0) + stats.get(
                "fuzzy_ancestor_matches", 0
            )
            logger.info(
        ...
        if expression_matcher is not None:
            ...
            try:
                ...
                n_corrected = sum(
                    1 for t in expression_match_types if t == "expression_corrected"
                )
                n_expr_only = sum(
                    1 for t in expression_match_types if t == "expression_only"
                )
        ...
        try:
            ...
            if expression_confidence is not None:
                adata.obs["morphology_confidence"] = expression_confidence.astype(
                    np.float32
                )
            ...
            if ancestor_distances is not None:
                n_direct = int(
                    (
                        (ancestor_distances == 0) & adata.obs["has_morphology"].values
                    ).sum()
                )
                n_ancestor = int(
                    (
            ...
            logger.info(
                ...
                + (
                    f" [{n_direct} direct, {n_ancestor} via ancestor]"
                    if ancestor_mapper
                    else ""
                )
            )
    ...
    def _add_contact_graph(
        ...
        try:
            ...
            adata.obsp["contact_adjacency"] = csr_matrix(
                (n_cells, n_cells), dtype=np.float32
            )
            adata.obsp["contact_binary"] = csr_matrix(
                (n_cells, n_cells), dtype=np.float32
            )
    ...
    def _add_lineage_proximity_graph(
        ...
        if len(unique_ancestors) < 2:
            ...
            adata.obsp["lineage_proximity"] = csr_matrix(
                (n_cells, n_cells), dtype=np.float32
            )
            return

        # Build ancestor -> cell indices mapping
        ancestor_to_cells: Dict[
            str, List[Tuple[int, int]]
        ] = {}  # ancestor -> [(cell_idx, distance)]
        ...
        if data:
            ...
            has_contact = adata.obs.get(
                "has_true_contact", np.zeros(n_cells, dtype=bool)
            )
            n_train = int((has_contact & has_proximity).sum())  # Have both: can train
            n_predict = int(
                (~has_contact & has_proximity).sum()
            )  # Have proximity but no contact: can predict
    ...
    def _add_cshaper_spatial(self, adata: ad.AnnData) -> ad.AnnData:
        ...
        try:
            ...
            adata.obsm["X_cshaper_spatial"] = np.full(
                (n_cells, 3), np.nan, dtype=np.float32
            )
    ...
    def _add_cshaper_metadata(
        ...
        if include_morphology and "has_morphology" in adata.obs.columns:
            ...
            if n_with_morphology > 0:
                ...
                cshaper_info["mean_volume"] = float(
                    adata.obs.loc[valid_mask, "cell_volume"].mean()
                )
                cshaper_info["mean_surface"] = float(
                    adata.obs.loc[valid_mask, "cell_surface"].mean()
                )
                cshaper_info["mean_sphericity"] = float(
                    adata.obs.loc[valid_mask, "sphericity"].mean()
                )
            ...
            if "morphology_ancestor_distance" in adata.obs.columns:
                ...
                cshaper_info["n_direct_morphology_matches"] = int(
                    (valid_distances == 0).sum()
                )
                cshaper_info["n_ancestor_morphology_matches"] = int(
                    (valid_distances > 0).sum()
                )
                if len(valid_distances) > 0:
                    cshaper_info["mean_ancestor_distance"] = float(
                        np.mean(valid_distances[valid_distances >= 0])
                    )
        ...
        if include_contact_graph and "contact_binary" in adata.obsp:
            ...
            cshaper_info["mean_contact_degree"] = float(
                contact_mat.sum() / max(adata.n_obs, 1)
            )
