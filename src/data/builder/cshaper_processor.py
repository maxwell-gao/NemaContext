...
def embryo_time_to_cshaper_frame(time_min: float) -> int:
    ...
    # Linear mapping
    fraction = (time_min - CSHAPER_START_TIME_MIN) / (
        CSHAPER_END_TIME_MIN - CSHAPER_START_TIME_MIN
    )
...
def cshaper_frame_to_embryo_time(frame: int) -> float:
    ...
    return CSHAPER_START_TIME_MIN + fraction * (
        CSHAPER_END_TIME_MIN - CSHAPER_START_TIME_MIN
    )
...
class AncestorMapper:
        ...
        if "/" in lineage:
            for alt in expand_slash_lineage(lineage):
                ...
                if "x" in alt_norm.lower():
                    for exp in expand_uncertain_lineage(alt_norm)[
                        : self.max_fuzzy_expansions
                    ]:
    ...
    def get_mapping_stats(self, lineage_names: List[str]) -> Dict[str, any]:
        ...
        return {
            ...
            "max_distance": int(np.max(valid_distances))
            if len(valid_distances) > 0
            else 0,
...
class ContactLoader:
    ...
    def load_sample(
        ...
        logger.debug(
            f"Loaded contact matrix for {sample_id}: {result.shape}, {len(frame_matrices)} frames"
        )
        return result
    ...
    def get_consensus_contacts(
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            mean_matrix = np.where(
                count_matrix >= min_samples, sum_matrix / count_matrix, np.nan
            )
    ...
    def _build_adjacency_gpu(
        ...
        large_contacts = [
            (n, r, c, v) for n, r, c, v in contact_info if n >= large_threshold
        ]
        small_contacts = [
            (n, r, c, v) for n, r, c, v in contact_info if n < large_threshold
        ]
    ...
    def get_cell_timeseries(
        ...
        if sample_id is not None:
            ...
            return {
                ...
                "volume": vol_df[cell_name].values
                if cell_name in vol_df.columns
                else np.array([]),
                "surface": surf_df[cell_name].values
        ...
        volumes = np.array(
            [np.mean(volume_by_frame.get(f, [np.nan])) for f in all_frames]
        )
        surfaces = np.array(
            [np.mean(surface_by_frame.get(f, [np.nan])) for f in all_frames]
        )
...
class StandardSpatialLoader:
    ...
    def _load_frame_coords(
        self, frame: int, embryo_idx: int = 0
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def get_spatial_coords(
        ...
        logger.debug(
            f"Found coordinates for {n_found}/{n_cells} cells at frame {time_frame}"
        )
        return coords
...
class Segmentation3DLoader:
    ...
    def compute_shape_descriptors(
        ...
        avg_face_area = (
            self.VOXEL_SIZE_UM[0] * self.VOXEL_SIZE_UM[1]
            + self.VOXEL_SIZE_UM[1] * self.VOXEL_SIZE_UM[2]
            + self.VOXEL_SIZE_UM[0] * self.VOXEL_SIZE_UM[2]
        ) / 3
...
class CShaperProcessor:
    ...
    def __init__(self, data_dir: str = "dataset/raw"):
        ...
        self._consensus_morphology_cache: Optional[
    ...
    def morphology_loader(self) -> MorphologyLoader:
        ...
        if self._morphology_loader is None:
            ...
            self._morphology_loader = MorphologyLoader(
                self.cshaper_dir / "VolumeAndSurface"
            )
    ...
    def spatial_loader(self) -> StandardSpatialLoader:
        ...
        if self._spatial_loader is None:
            ...
            self._spatial_loader = StandardSpatialLoader(
                self.cshaper_dir / "Standard Dataset 1"
            )
    ...
    def segmentation_loader(self) -> Segmentation3DLoader:
        ...
        if self._segmentation_loader is None:
            ...
            self._segmentation_loader = Segmentation3DLoader(
                self.cshaper_dir / "Standard Dataset 2"
            )
    ...
    def get_contact_timeseries(
        ...
        areas = np.array([np.mean(contact_values.get(f, [0.0])) for f in frames_sorted])
    ...
    def _build_consensus_morphology_cache(
            ...
            for cell in all_morph_cells:
                try:
                    ...
                    if len(ts["volume"]) > 0:
                        ...
                        if not np.isnan(vol) and not np.isnan(surf) and surf > 0:
                            sphericity = np.power(36 * np.pi * vol**2, 1 / 3) / surf
                            consensus_cache[cell] = (
                                vol,
                                surf,
                                float(np.clip(sphericity, 0, 1)),
                            )
                except Exception:
                    pass
    ...
    def get_spatial_coords(
        ...
        if embryo_times is not None:
            frame = int(
                np.median(
                    [
                        embryo_time_to_cshaper_frame(t)
