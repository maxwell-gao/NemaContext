...
class EmbryoViewDataset(PatchSetDataset):
    ...
    Local patches remain observation views; the biological target is the whole-window
    future developmental state summarized over all cells in the paired future window.
    """
...
def collate_embryo_view(batch: list[dict[str, torch.Tensor]]) -> dict[str, torch.Tensor]:
    ...
    output["future_spatial_extent"] = torch.stack(
        [item["future_spatial_extent"] for item in batch]
    )
    output["future_split_fraction"] = torch.stack(
        [item["future_split_fraction"] for item in batch]
    )
