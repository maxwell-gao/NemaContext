...
class ContactGraph:
    ...
    def adjacency(self) -> csr_matrix:
        ...
        if self._adjacency is None:
            ...
            self._adjacency = csr_matrix(
                (data, (row, col)), shape=(self.n_cells, self.n_cells)
            )
    ...
    def subgraph(self, cell_indices: np.ndarray) -> "ContactGraph":
        ...
        new_features = (
            self.node_features[cell_indices] if self.node_features is not None else None
        )
...
class TimeDynamicContactGraph:
    ...
    def get_temporal_edges(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        edge_index = np.vstack([np.concatenate(all_src), np.concatenate(all_dst)])
    ...
    def forward(
        ...
        for i in range(n_cells):
            ...
            neighbor_transformed = (
                neighbor_features @ self.W_neighbor
            )  # (n_neighbors, out_features)
