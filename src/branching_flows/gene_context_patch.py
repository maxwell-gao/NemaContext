...
class LineageWholeEmbryoModel(nn.Module):
    """Lineage-first whole-embryo gene video model for raw Large2025 Stage 1."""
    ...
    def forward(
        ...
        for block in self.spatial_blocks:
            flat_x = block(flat_x, flat_mask)
        encoded_frames = flat_x.view(batch_size, n_frames, token_count, self.d_model)
        ...
        future_tokens = pred_future_seed + self.future_token_queries[query_rank]
        ...
        history_token_mask = valid_mask.view(batch_size, n_frames * token_count)
        pred_future_token_states = future_tokens
        for block in self.refinement_blocks:
            pred_future_token_states = block(pred_future_token_states, history_tokens, history_token_mask)
        pred_history_genes = self.gene_head(token_temporal)
